...
End-to-end symptom on Eric's first v0.9.70 test:
...
**Entity-duplication audit and dedup defenses.** Eric reported 61
...
### What I cleaned up on Eric's HA via API (one-shot)
...
For off-grid setups: Eric explicitly does not want TTS to ever hit
...
for Eric until he ever goes back on TOU.
...
Discovered empirically while diagnosing Piper for Eric: HA's
...
- New `degradeReason: 'flat-forecast' | 'no-tou-spread' | null` field. With Eric's flat $0.17 tariff (v0.9.58), the planner now correctly returns `degradeReason: 'no-tou-spread'` and `expectedSavingsUsd: 0` — instead of producing garbage savings numbers from imaginary spread.
...
Each fix touches numbers Eric sees on the dashboard or numbers the
...
load curve — Eric's real consumption is 30–90 kWh/day. The 3-day
...
`socStepPct = socStep * 5` implied ~20 kWh fleet capacity. Eric's
...
suspicious. Verified against live data from Eric's DPU `Y711FAB59J234000`:
...
to a 12¢ fallback. None matched Eric's actual APS plan (flat $0.17/kWh).
