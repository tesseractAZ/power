...
const MUTANTS = [
  ...
  {
    id: 'ii. \u2605\u2605 the Cores\u2019 share is not capped at what their inverters report',
    file: MODEL,
    find: '      coresToHouseW = Math.min(load - gridShare, acOut);',
    to: '      coresToHouseW = load - gridShare; /* MUTANT */',
    why: 'Any meter lag between the main and the panel is attributed to the batteries as delivery they never made.',
  },
  {
  ...
  {
    id: 'iv. \u2605\u2605 the server\u2019s importWatts (which counts disconnected slots) feeds the Batteries edge',
    file: MODEL,
    find: '  const gridToCoresW = acIn;',
    to: '  const gridToCoresW = grid?.importWatts ?? acIn; /* MUTANT */',
    why: 'A non-member Core\u2019s draw is drawn into a Batteries node that does not contain it.',
  },
  {
  ...
  {
    id: 'vi. \u2605 the Grid node can be smaller than its own edges',
    file: MODEL,
    find: '  const gridSupplyW = Math.max(homeGridW, gridToCoresW + gridToHouseW);',
    to: '  const gridSupplyW = homeGridW > 0 ? homeGridW : gridToCoresW; /* MUTANT */',
    why: '"3.41 kW" on the Grid node beside a 16275 W edge leaving it.',
  },
  {
  ...
  {
    id: 'x. \u2605 meter noise under the edge floor is drawn as delivery',
    file: MODEL,
    find: '  if (coresToHouseW < FLOW_EDGE_MIN_W) coresToHouseW = 0;',
    to: '  /* MUTANT */',
    why: 'An idling inverter\u2019s 3 W beside a grid-fed house is drawn as the batteries feeding it.',
  },
  {
