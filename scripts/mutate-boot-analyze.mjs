#!/usr/bin/env node
/**
 * mutate-boot-analyze.mjs — committed mutation harness for v1.163.0: planner statistics
 * at boot.
 *
...
// Refuse to start over a leftover marker: the `originals` captured above would BE the
// mutated file, so every restore would write the mutant back and the run would report
// green against a tree that is already wrong.
...
// The `finally` at the end does not run on a signal. Without these, Ctrl-C during a
// multi-minute run leaves recorder.ts — the live boot path — holding a mutant.
