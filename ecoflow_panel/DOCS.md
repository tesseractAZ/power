...
2. **BMS coulomb-counter keys**: `fleet_battery_charge_wh` /
   ...
   **v1.103.0 — membership history (`membershipHistory.ts`).** Pool membership was resolved once from the live snapshot and applied across historical windows, so a query spanning a reconfiguration attributed one fleet's energy under another's roster — and `computeLocalPackRte` accumulated 77,218 Wh in against 92,676 Wh out (a round-trip ratio of 1.20, impossible) because the two legs of a day were measured over different batteries. A timestamped record of the connected-DPU set is now appended on EVERY rollup (`recordMembership` is idempotent; recording the STATE rather than the EVENT is what makes the first entry appear on a stable plant). `membershipVerdict(from,to)` returns `stable` / `changed` / **`unknown`** — a window predating the record is REFUSED, never assumed clean — and an empty fingerprint (panel unreadable) is never recorded. The pack-DC RTE excludes non-`stable` days and reports `excludedDays`; `computeTotals` declares its `membershipBasis`. Honest limit: history cannot be reconstructed backwards, so windows before this release report `unknown`.
...
- **The house panel is pinned, and the pin never moves by itself.** `resolveHousePanel(census,
  pin, singleStable)` (pure) → `pinned | missing | ambiguous | none`. `census` is the panels ON THE
  ACCOUNT (`SnapshotStore.accountPanels`: the identity census less any panel absent from
  `PANEL_ABSENT_LISTS` = 3 consecutive device lists — the device map never forgets a device). A pin
  whose panel is on the account is `pinned`; off the account it is `missing` (no automatic re-pin:
  one partial list after a restart used to hand the role, and the pending revert, to the other
  panel for good). No pin and one panel standing alone for `FIRST_PIN_LISTS` = 2 lists: pin it; two
  or more: `ambiguous`. `SnapshotStore` persists the pin and every panel's last connected-source
  roster (`house-panel.json` beside the DB in production; `HOUSE_PANEL_PATH` overrides; in memory
  elsewhere) and stamps `DeviceSnapshot.housePanel` on the house panel, `housePanelMissing` (the
  pinned serial) on every panel while it is missing, and `lastRoster` on each panel (carried,
  restored on first sight). The flags travel with the device into the analytics worker and the web.
- **`findShp2(devices)`** returns the flagged panel, or **undefined while that panel has no
  projection or the pin is missing** (never the other panel); with nothing flagged, the lowest
  serial (unchanged). `houseSnOf` names the house serial (flagged, else the missing pin, else the
  fallback); `secondaryPanels` lists every other panel by identity, projected or not;
  `panelRoster(panel)` is its live connected sources, else `lastRoster`. Every
...
- **Writes.** `multiPanelWriteBlock` refuses supervised apply writes while the pin is missing (any
  panel count) or two panels stand with none flagged; every write targets `findShp2`.
  `POST /api/house-panel {sn}` (write-auth) pins a panel on the account and refuses to MOVE an
  existing pin while the arbitrage reserve is raised, a reserve apply is unconfirmed, the
  force-charge ceiling is not yet restored, a night-charge write is in flight, a Charge Now write
  awaits readback, or a house slot is force-charging. The `shp2-multi-panel` alert and the
  dashboard's `HousePanelPin` prompt stand while ambiguous or missing.
- **Grid verdict per pool.** `livePoolGridBackstop(devices, sn)` = the resolver over
  `panelScopedDevices` (that panel and its roster's Cores): its own gridSta, main-line flow,
  Cores' ac_in, veto, floor and discharge. Every pool alarm (house and secondary: on-screen via
  `computeAlerts(…, poolGrid)`, the SoC ladders, the runway alarms) reads its own pool's verdict
  on a multi-panel plant; with one panel it returns `liveGridBackstop`, unchanged. The plant
  verdict (`liveGridBackstop`) backstops only when every pool is backstopped.
...
- **The secondary runway** (`panelRunway.ts`) is the pool's energy against its measured drain:
  ...
  reports), sampled each 2-min runway tick (before, and independent of, the analytics worker's
  report), averaged over 30 min once the window spans 10 min; a null reading clears the window.
  Unavailable on a stale panel reading — and an unavailable figure is not fed to the alarm, which
  holds its state instead of re-arming; no depletion under 50 W or beyond 24 h; a pool at or under
  its reserve reads at the floor whatever the drain.
- **Fallback SoC.** `housePoolFallbackSoc` = `homeFleetMeanSoc` with one panel; with more, only
  the house panel's own Cores (`panelMeanSoc` over `panelRoster`, so a house panel dark since a
  restart still reads its own) — null when no roster is known, never the union. Secondary ladders
  (every secondary panel, projected or not) fall back to their own Cores, and a secondary panel
  with no projection raises `reserve-alarm-blind-<serial>` from when it was first listed.
...
- **Saved grid readings.** A persisted not-OK reading is pruned only after its panel is absent from
  `PANEL_ABSENT_LISTS` consecutive panel-naming lists (one partial list is not evidence).
...
**v1.157.0 — an idle device's held collapse no longer votes for a heal.** The heal
...
`gridSta`, all keep vetoing on the last not-OK reading — across an add-on restart too
(v1.180.0, "Surviving a restart" below). Every way of lifting the veto on silence that was
...
- **The cloud itself is unreachable** (an outage that also takes the internet down). No
  ...
  (`GridBackstopInput.persistedGridAbsent`, registered in `index.ts`).

The hand-over is per serial and needs evidence. A persisted reading is served until **its own
panel** appears in a list; from then on it lives on the panel's device. A list that comes back
empty or without the panel is a cloud-side glitch, not evidence (v1.145.0), so it leaves the
reading standing. A list showing a **different** panel is evidence the old one was replaced or
removed, and prunes its entry, so a stale reading cannot veto a later restart made while the
...
report; while the panel stays dark, the dashboard's "Grid is back — clear" (v1.184.0,
`POST /api/grid-veto/clear`) sets it aside until the panel's next reading. The reason marks it "(last reading from before a restart; panel offline)",
...
`today` walks past hours with weather and is always live; `recent7d`
(`CURTAIL_HISTORY_DAYS = 7`) covers the 7 finished local days before today. An hour is
weather-verified when the analytics worker's own Open-Meteo cache (`past_days=7`, UTC days)
holds it with GHI ≥ `CURTAIL_MIN_GHI_WM2` and the posterior has support; any other hour takes
the heuristic path. **Frozen days (v1.186.1, `curtailmentFreeze.ts`).** A finished day is
walked only until it *settles*: the weather in hand was fetched at least
`CURTAIL_SETTLE_LAG_MS` (12 h) after the day ended, so none of its hours is a forecast value
and the provider's past-hour revisions are done (past hours are stitched from each model run's
first hours; the slowest listed model is ingested ~7.2 h after initialisation); every hour of
the day is in that cache with a value the provider sent (no `radiationMissing`); and a posterior
exists. The walk that first finds a day settled freezes its kWh and curtailed hours (hour of day,
surplus W); from then on `recent7dKwh`, `recent7dHoursCount` and `hourlyHistogram` read the
frozen record, whatever the posterior or the weather cache later says. A stale cache (every
fetch failing) keeps its old `fetchedAt`, so the days after it stay live. Frozen days persist in
`curtailment-days.json` next to the database (atomic; `CURTAILMENT_DAYS_PATH` overrides; memory
only outside the add-on) and are pruned once they leave the window, so the figure changes
between midnights only while an unsettled day is still being estimated, and by one step at local
midnight when the window slides. `CurtailmentReport` fields: `active`, `currentSurplusW`,
...
`todayHours[]`, `recent7dKwh`, `recent7dHoursCount`, `recent7dSettledDays` (days read
frozen), `hourlyHistogram[]` (hour-of-day siting for opportunistic loads), and `opportunisticLoads[]` (static
...
**Critical is never silenced or demoted.** `ENERGY_STATE_FAMILIES` are exempt from all four rules (a fast clear is a genuine recovery, not jitter): `backup-soc`, `shp2-below-reserve`, `shp2-near-reserve`, `soc-low`, `forecast-runtime`, `offline`, `stale`, `forecast-soc-dip`, `reserve-alarm-blind`. Bench-spare churn rolls up under separate `offline-spare` / `stale-spare` families so it can still be auto-tuned without poisoning home-device stats.

**v1.95.0 — off-panel annunciation demotion.** Annunciation used to be gated solely by the static `SPARE_DPU_SNS` literal, which describes the bench only at the moment it was written; the 2026-08-20 reconfiguration inverted it, so a benched Core chimed at full volume (19 of 19 pushes and 48 of 67 s of audio in one window) while a live home Core sat inside the literal. A DPU absent from a **non-empty** SHP2 roster is now demoted to `annunciate:false`, with three guards, each mutation-proven load-bearing (`scripts/mutate-off-panel-annunciation.mjs`, 6/6): **hysteresis** — `OFF_PANEL_DEMOTE_TICKS` (3) consecutive absences to demote but **one** sighting to re-arm, so a flickering `isConnect` can never silence a live home Core; **empty roster demotes nobody**, because an unreadable panel is not evidence; and a **thermal-critical carve-out** — a `critical` alert in category `Thermal` is never demoted, since a bench pack that is overheating must page regardless of where it is wired. Pure cores `advanceOffPanelStreaks` / `shouldDemoteAnnunciation` are exported for tests.

**v1.102.0 — pack identity follows the hardware.** Alert ids are keyed `(chassis, slot)`, so a physical pack swap silently re-points a live id at different hardware — on 2026-08-20 `vdiff-crit-<sn>-1` carried through with no resolve and no re-raise, its detail changing from "Deviant cell #31" to "cell #32" mid-episode, merging two batteries into one cleared-alert record. Every pack-scoped alert now carries `sourcePackSn` (the BMS reports it on every read). When that serial changes under a live id, `alertMonitor` retires the episode through the SAME helper a natural clear uses — honest duration, cleared-log record — and the rising-edge pass opens a fresh one. `/api/warranty-export` matches on the pack serials a device currently holds, and `?packSn=` narrows the bundle to one pack wherever it has lived; records predating the field still fall back to the chassis match. **Ids are deliberately unchanged**: re-keying them would force a one-time fleet-wide resolve/re-raise and break the persisted notified-state dedup, a large blast radius for a narrow correctness problem.

**v1.101.0 — the quiet half.** Demoting off-panel hardware removed the noise but left the inversion: the defective warranty pack, sitting on a bench chassis, was the one battery nobody was paged about. `pack-defective-<sn>-<pack>` is a STANDING alert (one per device+pack, deduped to a single push, resolving on replacement) raised on a two-leg signature — a BMS protection latch (`packLatchSignature`: SoC ≥ 20 pts below the sibling median, < 25 W exchanged, siblings ≥ 100 W) AND a deviant cell at least `DEFECTIVE_PACK_MIN_DEVIANT_MV` (50 mV) from the pack median. The second leg is a threshold, not a null check, because `packCellForensics` names its most-deviant cell even on a matched pack. Both demotion paths (the bench-spare stamp in `alerts.ts`, the off-panel demotion in `alertMonitor.ts`) consult one shared predicate `isNeverMutedAlert` — critical Thermal, or a confirmed-defective pack — so they cannot drift apart. Harness `scripts/mutate-never-muted.mjs` (8/8). In `dispatch()`, a critical alert is never suppressed even if its family latch was set from info/warning noise.

**v1.108.0 — the LATCH.** The first day the TOU window let the bench bank charge (2026-08-24), the alert fired and resolved three times: leg 3 of the live signature (siblings ≥ 100 W) tracks the charger's burst duty cycle, so the alert cleared every time a burst ended — a [High] push plus a Resolved push per burst, a cry-wolf cadence on the one alert exempt from every mute path. "Confirmed defective" is a diagnosis, not a live condition. `defectivePackLatch.ts` records the first full-signature observation per PHYSICAL pack serial (`/data/defective-pack-latch.json`; the 08-20 swap proved faults travel with the pack, so the key is packSn, never the slot), and from then on the standing alert is emitted whenever that pack is present — legs or no legs — with a detail that names the confirmation date and the captured evidence. It clears when the pack leaves the fleet (emission stops at once; the record itself retires 48 h later so a replacement with the same SN does not inherit the verdict) or via the operator's `POST /api/defective-packs/clear?packSn=…` (write-auth). `GET /api/defective-packs` lists the latched records. Without a reported packSn the legs-only v1.101.0 behavior stands — nothing may latch on a slot alone. Confirmations survive restarts; presence tracking is in-memory (a restart merely restarts the 48 h absence clock). Harness mutants vii/viii pin the two silent failure modes: quiescent-emission dropped (the flap returns) and confirmation-never-recorded (latch inert).
...
`fv.normalized`. Any error → `null` (SGD step is simply skipped). v1.186.0: it runs
detached, after the tick's dispatches, so its four worker reports never delay a push or an
announcement (before, the rising-edge loop awaited it).
...
**v1.186.1 — the 7-day curtailment figure stops rewriting history.** `computeCurtailment` re-walked all 168 past hours on every refresh through the current Bayesian posterior and the worker's current Open-Meteo cache, so `recent7dKwh` (`ecoflow_pv_curtailment_kwh_7d`, the card's "Past 7d lost") and the 7-day histogram moved with no new curtailment in the window (2026-09-25: 12.6 → 10.5 → 16.09 kWh). `curtailmentFreeze.ts` freezes a finished local day the first time a walk finds it settled — `curtailmentDaySettled`: the weather's `fetchedAt` ≥ day end + `CURTAIL_SETTLE_LAG_MS` (12 h; the cache mixes forecasts and past-hour estimates, an hour is a past-hour estimate only if it had ended at `fetchedAt` — the realized-GHI capture's rule — a failed fetch keeps serving the stale cache, and past hours are revised until the model runs covering them are ingested, ≤ ~7.2 h per the provider's model metadata), all 24 hours present without `radiationMissing`, and a posterior (`bayes.hourly.length > 0`, so a model-less walk never freezes a week of zeros). The frozen record holds the day's kWh and curtailed hours (hour of day, surplus W); `recent7dKwh`, `recent7dHoursCount` and `hourlyHistogram` read it, only unsettled days are re-walked, today stays live, and the new `recent7dSettledDays` counts the frozen days. Persisted in `curtailment-days.json` beside the database (atomic temp + rename; `CURTAILMENT_DAYS_PATH` overrides; memory only when neither it nor `SUPERVISOR_TOKEN` is set), pruned to the window, invalid entries dropped on load; a failed save is logged once and retried on each refresh. `computeCurtailment` takes the worker's logger and an injectable clock for the live state and day walk (the report cache stays on the real clock). Entity ids are unchanged. Harness: `scripts/mutate-curtail-freeze.mjs` (8 mutants).

**v1.186.0 — fixes from the 09-24 log review, and solar headroom on long-gap nights.** (0) Long gap: the cost ceiling on a `longGapAhead` night is min(`ARB_COST_MAX_SOC_PCT`, 100% − `pessimisticPrePeakSurplus`) — the P10 PV surplus over forecast load from window close to the first on-peak hour (≤14 h), basis `p10` (`none` when unknown ⇒ the old full buy); non-long-gap nights unchanged. Boot: the alert monitor's first evaluation waits for `store.markFirstPollSettled()` (`hydrated`), bounded by `BOOT_HYDRATION_MAX_MS` (30 s); the broadcast's first tick routes a non-green level through the boot gates; the worker's first-snapshot gate needs projections; HA alarm counts wait for `alertsComplete` (≤90 s). (1) Readiness: each plan persists `cushionTroughSocPct` / `cushionLineSocPct` (the trough that set `cushionShortfall` and reserve floor + applied cushion; legacy-pct keeps the whole-house trough and floor + `cushionPct`); `scoreNightOutcome` grades `plan_traj_floor_breached` on them (0.05 %-pt tolerance; no trough ⇒ null, never graded on the disclosure trough `minProjSocPct`). `nightChargeGate` counts a stored trajectory flag only with the new trough column or a NULL `cushion_shortfall`; a pre-v1.186 row with `cushion_shortfall` 0 and no trough is set aside (`metrics.trajStrikesSetAside`); actual-night breaches still strike. New ledger columns `cushion_trough_soc_pct`, `cushion_line_soc_pct`. (2) Auto-tune: rollups count only annunciating episodes (`rollupScope` on the tracked alert; `autoTuneCounts` / `autoTuneClearCounts`); telemetry lines carry `scope:'annunciating'` and `replayTelemetryEvents` skips unscoped lines, lifting verdicts earned by the old counting (`liftedAutoTuneVerdicts`, logged at boot); dispatch rates the rules at max(own, exemplar) severity (`autoTuneDispatchVerdict`); every suppressed or demoted dispatch is logged with its rule and counts. (3) Worker: live-snapshot alarms `store.setAlerts` before any worker await; forecast, NWS storm-prep, curtailmentAlerts, baselineAlerts and forecastAlerts are last-good feeds under `ALERT_FEED_BUDGET_MS` (2.5 s; a timed-out read carries its last good value, never an empty list); onset pruning and the boot orphan sweep hold while a feed has not delivered (≤10 min); `captureLrFeatures` runs detached. `reports.buildReport` is single-flight by name + args (stuck after 5 min); a client retry joins the running computation and either answer settles the caller; the warm pass yields between reports; the charge-curve scan reads week slices aligned to 60 s buckets with a yield per slice (equivalence-tested); the HA state publish settles each report on its own (5 s, else last good ≤120 s, else null; runway null never publishes 999; the lighting posture publishes only with runway, forecast and curtailment present); report requests wait for the worker's first device snapshot (≤5 s once without devices). (4) Broadcast: kinds `condition` / `dedicated` / `test`; tests arm nothing and take no retry slot; only condition deliveries arm the same-level storm gap and cancel a condition's pending spoken retry; the boot baseline is a persisted condition record (`broadcast-last.json`) written on every adoption and marked heard only by a verified condition delivery; `system-audible-degraded` (warning) while usable MA targets < configured, 3-probe debounce; per-broadcast `usable/configured` logging; `/api/broadcast/status` adds `audibleConfiguredTargets`, `audibleDegraded`, `audibleUnusableTargets`, `lastBroadcastKind`, `conditionLevel`, `conditionSpoken`, `conditionAt`, `bootBaselineLevel`. (5) Force-charge: `verifyForceChargeOn` (one-shot, fresh readback only, a slot at the stop ceiling counts as applied) after `FORCE_CHARGE_ON_VERIFY_AFTER_MS` (6 min); one `onRetry` to the slots still OFF (grid present, vitals not red, write interlock honoured); then `onFailed` (warning push `night_charge_force_charge_on_failure`); OFF triggers are always evaluated first. State `forceChargeOnVerifiedAtMs`, `forceChargeOnLastAttemptMs`, `forceChargeOnRetries`, `forceChargeOnFailedAtMs`; `/api/night-charge/status` `actuation.forceChargeOnVerify`. Harnesses: `mutate-readiness-gate` (10), `mutate-alert-monitor` (16), `mutate-analytics-worker` (12), `mutate-broadcast` (12), `mutate-force-charge-on` (10).

**v1.185.1 — the speaker test.** `POST /api/broadcast/test` validated `level` against `CHIME_LEVELS`, which v1.59.0 made the five alarm rungs, so every documented level (and the empty-body default `red`) got 400 and a rung name was spoken as "All clear". It now uses `parseBroadcastTestLevel` (broadcast.ts: `BROADCAST_TEST_LEVELS` = red / yellow / green, default red). Harness: `scripts/mutate-broadcast-test-level.mjs` (3 mutants).

**v1.185.0 — two smart panels, supported.** The house panel is pinned (`resolveHousePanel`, persisted `house-panel.json`, `DeviceSnapshot.housePanel`) and `findShp2` returns only it — undefined while it has no projection or while the pin is missing from the account (the pin never moves by itself; `PANEL_ABSENT_LISTS`, `FIRST_PIN_LISTS`) — so a lower-serial second panel can no longer become the target of the SoC ladder, HA backup sensors or night-charge writes; every inline `find(kind === 'shp2')` resolves through it (web: `findHousePanel`). `multiPanelWriteBlock` stands only while two panels have none pinned; `GET/POST /api/house-panel` reports the pools and pins a panel (write-auth, refused mid-actuation). The grid resolver reads every panel: presence is Grid OK only if every fresh panel agrees and false if any fresh panel reports not-OK; `gridWatt` is summed over fresh panels; a panel freshly reporting no grid drops its own Cores' ac_in; the declared-grid veto is any panel's last not-OK reading (`panelVetoes`), clearable only when no vetoing panel is fresh; `atReserveFloor` is any panel's; every pool alarm reads its own panel's verdict (`livePoolGridBackstop` over `panelScopedDevices`) and the plant backstops only when every pool does. Panel rosters persist with the pin (`lastRoster`). Each secondary panel carries `-<serial>` reserve, source, circuit, reserve-blind and SoC-band alerts (`secondaryPanelAlerts`), its own spoken SoC ladder and a drain-measured runway alarm (`panelRunway.ts`, "at the current drain"). `housePoolFallbackSoc` keeps the house ladder's blind fallback on the house panel's own Cores; `panelLoad` and the web flow model sum every panel; bench-spare muting is disarmed only while a panel's roster is missing (`multiPanelRosterUnsound`). One panel: unchanged. Harness: `scripts/mutate-multi-panel.mjs`; nine harnesses repointed.
...
**v1.183.0 — the last three audit findings.** `web/src/cards/cardText.ts`: `outlookOf` (no projection → "—", neutral), `strategyExclusion` (`loadIsEnable: false` is the load-strategy flag from `LoadStrategyCfg`, not the relay — "not in the SHP2's load strategy", with the power drawn; no strike-through; listed after the shed order, unranked and untiered), `remainSuffix` (the DPU countdown is "to full" / "to empty" by `batAmp` sign; `packCountdownLabel` does the same for the Thermal page's pack readout by pack input vs output, and the telnet tag is `GEN.n.TTF.MIN` while charging, `RUN.MIN` otherwise). `web/src/charts/axisFormat.ts`: `kwTick` for the Forecast chart's labelled kW axis and `compactTick` for the trend charts (two decimals, trimmed: recharts steps in 50 W multiples), whose unit moved from every tick to the axis label. Harness: `scripts/mutate-audit-last-three.mjs` (8 mutants).

**v1.182.0 — dashboard figures cover the home.** `computeTotals` adds `fleet.homeCoverage` (home Cores and the panel only, averaged per device so a dark home Core — counting 0 — weighs as one device, not one series against ~14) and `fleet.panelLoadCoverage` (lowest across panels; null with no panel); the Today card reads them ("not measured today" at 0). `computeDegradation`, `computeChargeCurveFingerprint` and `computeThermalEvents` tag packs `home` (`tagHomePacks`, panel membership); the Battery page scopes fleet capacity, the SoH range and the charge-curve/thermal lists to home packs (`web/src/cards/batteryScope.ts`). The Alerts badge (`alertBadge`) counts a learned CRITICAL. `energyFlowModel.membershipUnknown` keeps the Batteries title honest while the panel's source list is unknown. The night-charge status carries `reserveWriteMaxPct`; the card shows the clamped write (`reserveWriteLabel`) and clears an actuation banner by `actuationBannerVisible` (6 h after the CONFIRMED revert, `revertVerifiedAtMs`; a restore the panel has not confirmed while retrying or escalated — `revertUnconfirmed` — stays up and is not called "Completed"). Harness: `scripts/mutate-dashboard-audit-four.mjs` (10 mutants).

**v1.181.0 — a replayed Core proves no grid flow; the runtime alert's grid rule runs on the main thread.** `DeviceSnapshot.contentChangedAtMs` (set by `SnapshotStore` from `dpuContentWitness`, carried through `setDeviceList`) replaces `lastTelemetryAtMs` in the Core-import freshness gate, and `resolveGridBackstop` zeroes Core import while the panel freshly reports no grid. `computeForecastAlerts` (analytics worker) no longer resolves the grid; `alertMonitor` applies `applyRuntimeGrid` with its live verdict. Harness: `scripts/mutate-core-replay-runtime-grid.mjs` (7 mutants).
...
**v1.176.0 — dashboard freshness.** The header age and link pill read `lastTelemetryAtMs` (new server clock, content-only) capped at a panel's `contentStaleSinceMs`, over the panel (by identity) and online connected Cores, on the server's clock (`serverNowMs` stamped per WebSocket frame, minimum offset per socket); the pill adds a **stale** state. Runway, Today and Curtailment poll through `usePolled` and mark a payload that has missed its polls; Today and the Solar tab drop a finished day's totals (`dayEndMs`). The Solar tab and circuit popup no longer store an HTTP error body as data (each crashed the whole dashboard), and a failed Solar history series keeps the last good chart. Harness: `scripts/mutate-dashboard-freshness.mjs` (23 mutants).

**v1.175.0 — the Energy flow card and `panel_load`.** The card draws the grid to its destinations (`gridToCoresW` into the Batteries, `gridToHouseW` into Loads) instead of drawing the whole main-line total into the Batteries with the only Loads edge leaving them; Loads edges are attributed from the fresher meter (the Cores' `acOut`) and always sum to the Loads node, instead of `Math.max(load, acOut)`; a frozen or silent panel reads "—"; the Loads subtitle counts paired circuits. The recorder's `panel_load` sum starts at null, so a silent panel writes no row instead of a fabricated 0 W, and the runway's carried-forward recent load expires two hours after the last real reading (`RUNWAY_LOAD_CARRY_MAX_MS`). Harness: `scripts/mutate-energy-flow.mjs` (15 mutants).
...
- **Header age and link pill** (since **v1.176.0**, pure logic in `web/src/freshness.ts`, run by `server/test/dashboardFreshness.test.ts`). "updated N ago" is the age of the OLDEST home reading on the server's clock (browser now − `clockOffsetMs`), re-rendered by a 5 s ticker. Home = every Smart Home Panel (by projection, or by `productName` before its first quota) plus the ONLINE SHP2-connected Cores (every online DPU on a cold boot). A reading's time is the device's `lastTelemetryAtMs` — a server clock moved only when telemetry content lands (REST quota or MQTT delta; not a `/status` flip, a failed poll or an empty payload; carried through the `/device/list` rebuild) — capped at `contentStaleSinceMs` for a panel replaying a cloud shadow; a device with no telemetry since the add-on started counts as never reported. The pill reads **live** only when the socket is open AND the oldest reading is under `TELEMETRY_STALE_MS` (3 min, the alarm engine's "Telemetry stale" threshold), **stale** when open but older, **linking** while connecting, **offline** with no socket. Before v1.176.0 the header printed `snapshot.generatedAt` (bumped by every poll FAILURE) and the pill was the socket's readyState alone — open, and green, through the 2026-09-22 03:17–03:28 cloud outage.
...
| Option / Env | Default | Schema | Purpose |
|--------------|---------|--------|---------|
| `RECORDER_RETENTION_DAYS` | `30` | `int(7,3650)` | (v1.51.0; ceiling raised v1.108.0) Raw-samples retention in days for the SQLite recorder. Runtime clamps to [7, 3650] and falls back to 30 on a malformed value; the durable night-charge ledger and lifetime totals are never pruned regardless. |
...
## 15. Night-Charge TOU Arbitrage Advisor

*Tariff model, worst-case sizing brain, learning ledger, completion-gated scoring, the frozen write-readiness gate, and the supervised bounded write — the engine that recommends (and, when the owner enables a write mode, delivers) a cheap-overnight grid buy sized to hold an outage cushion above the reserve floor.*

On a day a battery shortfall is anticipated, the advisor recommends buying the *right* amount of grid energy in the cheap APS R-EV overnight window (23:00–05:00 Mon–Fri) so the home (a) never imports at the 4–7pm on-peak and (b) carries an **outage cushion** above the reserve floor into the next day. It is framed as much a *resilience* feature as a cost feature. The write posture is owned by `NIGHT_CHARGE_MODE`: **`advisory`** (default) never issues a device write — the recommendation is published as HA entities, an API endpoint, a nightly notification, and TUI/web panels, and an owner-authored HA automation decides whether to act; **`supervised`** performs one announced, cancellable, bounded reserve write per charge night with an automatic revert (§10); **`auto`** is honored only after the readiness gate graduates on actuated-night evidence and operates as supervised until then. In parallel, every night's plan is frozen into a durable ledger and scored against measured reality, and a fail-closed readiness gate reduces that record into a single "is the accuracy proven enough for unattended writes?" verdict.

The subsystem spans six modules, all shipped incrementally (v1.36.0 tariff → v1.37.0 sizing brain → v1.38.0 ledger + wiring + surfaces → v1.39.x scoring/gate hardening → v1.49.0 sizing-model correction → v1.50.0 supervised write + gate v2):

| Module | Role |
|---|---|
| `tariff.ts` | Declarative multi-period, seasonal, timezone-resolved TOU model + `rateAt` resolver (pure) |
| `nightChargeAdvisor.ts` | Pure sizing brain (`computeNightChargePlan`), input assembly, outcome scoring math, actuated-delivery helpers, night-clock bounds, plan holder |
| `nightChargeGate.ts` | Pure write-readiness reduction (v2: actuated-night evidence) over the ledger + readiness holder |
| `nightChargeActuator.ts` | Pure supervised-write decision core: mode resolution, arming from the announced plan, apply/revert decisions (all clocks injected) |
| `recorder.ts` | `night_charge_ledger` / `night_charge_calibration` durable tables + injection-safe upsert |
| `index.ts:2320–3400` | Orchestration: 30-min recompute tick, ~21:30 evening job, backfill scorer, premature-capture repair, 60 s actuator tick, `/api/night-charge/status`, `/api/night-charge/cancel` |
...
The charge-power cap credits only the **remaining** window when computed mid-window: the 30-min recompute tick runs around the clock and `resolveCheapWindow` deliberately walks back to the true window start for display honesty, so crediting the full window length would over-credit up to ~6 h × `chargeCapKw` of nonexistent lift and present an undeliverable buy as fully meeting the cushion (v1.39.0). The horizon is already trimmed to ≥ the current hour upstream. v1.49.0: the cap is **charge-only** — `chChargeWatt` limits grid→battery power and window house load rides the grid service feed, never debiting the charge budget; while the charger runs, the home is on grid bypass, so charging hours do not drain the pack either (the prior shared-budget model double-counted window load and under-sized buys ~5×, falsified by measured window imports sustaining ~2× the cap). Only `legEff` of what reaches the charger is stored.
...
- **DISPOSITION** (v1.132.0, both NULL on rows written before it):
  - `arm_disposition` — why an armed night never became an actuation. `actuated` alone is over-loaded: `NULL` covers at least five dispositions (superseded by a later plan, held below the minimum buy, no window resolved, advisory mode, apply guards refused). The 2026-08-29 row sat at `actuated=NULL` with `buy_kwh=36` and read like a failed 36 kWh buy when it was a routine Sat/Sun shared-window supersede — a log line recorded it, the ledger did not.
  - `cost_ceiling_basis` — `'pv-headroom' | 'max-soc' | NULL`. **The field that makes cost mode auditable.** `objective` only records which mode was *configured* plus buy/no-buy; `costModeTargetKwh` floors at the resilience answer and raises only under a strict inequality, so a row labelled `cost_arbitrage` can carry **zero** cost-mode contribution. This value was computed and thrown away, which is why "is cost mode actually doing anything" could not be answered from the ledger.
...
**★ Never pruned.** The table is created alongside `lifetime_totals` and is never referenced by the hourly `samples` prune — the multi-month verification record the write-readiness gate reduces over **must survive the samples retention** (design §0.1, binding; 30 days by default, `RECORDER_RETENTION_DAYS` since v1.51.0). The companion `night_charge_calibration` singleton (`id=1 CHECK`; seasonal cushion learner / buy de-bias / EV-tail inflation state, design §3.4) has schema and read/write API but **no live producer or consumer yet** — the learner is a later increment.

**Write path.** `nightUpsert` builds a dynamic `INSERT … ON CONFLICT(plan_date) DO UPDATE` where column names come *only* from a frozen whitelist (`NIGHT_LEDGER_COLUMNS`), never caller keys — that is what makes the interpolation injection-safe. Only columns present and not `undefined` are written, so a partial outcome merge preserves every frozen plan column; prepared statements are cached per column-shape; JS booleans are coerced to INTEGER 0/1 (`node:sqlite` rejects a boolean bind). `recordNightPlanRow` (`index.ts:2668`) coerces nullable plan numerics to 0 for the non-null plan columns (`target_soc_pct`, `buy_kwh`, `required_extra_kwh`) but *omits* `min_proj_soc_pct` when null so the sentinel survives. The synthetic SN `night_charge` is reserved for optional disposable chart-overlay samples (subject to the configured samples retention) and joins the restart-gap exclusion list (like `weather`/`forecast`) so off-cadence wall-clock rows can never mask a real telemetry stall; the durable ledger itself never rides the `samples` table.
...
- **Forecast pool** — current-algo, outcome-captured, `confidence_tier='forecast'` rows. Feeds PV/load accuracy and band coverage (those columns are recorded on every captured night regardless of the realized-need `scored` flag).
...
PV/load MAE and signed bias, the autocorrelation-adjusted effective N, and the MNAR exclusion fraction remain computed and published as diagnostics (same definitions as v1) but no longer gate — the five criteria above are the graduation contract. Accuracy remains **normalized MAE + a separate signed bias, deliberately not r²**: r² is variance-driven and inflated by monsoon swings — a +15–20% biased-but-correlated forecast passes r² ≥ 0.80 yet mis-sizes the buy.
...
`nightChargeGateFields` flattens the result for the state payloads; on a null readiness it emits `night_charge_readiness: 'unknown'`, **`night_charge_write_ready` strictly `false`**, and every diagnostic null. v2 note: `night_charge_plan_nights_scored` counts scored **actuated** nights.

---

### 8b. Supervised write mode (`nightChargeActuator.ts`, v1.50.0)

When `NIGHT_CHARGE_MODE` is `supervised` (or `auto`), each charge night runs one bounded, announced, cancellable, auto-reverting reserve write. The decision core is pure (`decideActuation` — all clocks injected, unit-tested against every guard); the integrator executes decisions through the audited write helper and persists every transition to a restart-surviving state file (`.night-charge-actuation.json`, atomic write, in-memory mirror).
...
4. **Score.** The scorer's actuated branch (§7) measures the delivered buy as window grid import minus the concurrent house pass-through (`actuatedDeliveredKwh`) and records `delivered_kwh`, feeding the gate's actuated pool. *(Corrected v1.132.0: this previously also described deriving the realized-need counterfactual via `actuatedRealizedNeedBuyKwh`. That helper still exists in `nightChargeAdvisor.ts` but is **no longer on the scoring path** — it is exercised only by tests. Do not reason about live scoring from it.)*
...
**`GET /api/night-charge/status`**: `{ enabled, mode, window, reserveFloorPercent, confidence, notify:{hour,minute,lastNotifyDay}, plan, readiness, actuation, recentOutcomes }`. `mode` is the resolved `NIGHT_CHARGE_MODE`; `actuation` is tonight's supervised-write record (armed/applied/cancelled/reverted + `cancelDeadlineMs`, §8b). `reserveFloorPercent` is read live from `projection.backupReserveSoc` — the same field the floor alarm defends. `recentOutcomes` is an in-memory mirror of the last 7 ledger days refreshed by the timers, so the request handler never touches the DB inline (CWE-770). No auth: read-only, exposes no secrets, actuates nothing.
...
| `ARB_CHARGE_CAP_KW` | `0` | float 0–50 | Grid-side charge-power ceiling for the feasibility cap. `0` = AUTO (v1.173.0): connected Cores × `FORCE_CHARGE_PROVEN_KW_PER_SLOT` ÷ √RTE (~5.9 kW per Core); > 0 is a fixed override. Home load comes off separately via `ARB_GRID_INPUT_CAP_KW`. Sizing only, no device is ever set |
...
Env-only knobs (not in the options form): `NIGHT_CHARGE_LATCH_PATH` (latch file location; defaults beside the DB), `EV_MAX_LOAD_W` (default `11520` — the per-hour EV block cap, shared with the EV detection engine), `ARB_LOAD_P90_CAP` (default `2.0`, `index.ts:3218` — the upper bound of the load-band widening; **if this is ever added to the options schema it MUST ship an `options:` default**, because `Number('')` is `0` and an unset option would pin the load band to zero), and the shared efficiency seams `DISPATCH_ROUND_TRIP_EFFICIENCY` (default `0.86`, clamped [0.8, 1]; the advisor's charge leg is its square root ≈ 0.927) and `RUNWAY_DISCHARGE_EFFICIENCY` (default `0.94`, clamped [0.8, 1]) — both injected from `analytics.ts`, never re-declared.
...
- **Hardware charge envelope.** Measured 2026-09-16/18: grid import pins at ~19.1 kW and the pack takes (19.1 − house) × ~0.93, up to ~17 kW across three Cores. Since v1.173.0 `ARB_CHARGE_CAP_KW` = 0 derives the cap from the connected Cores; the old 7.2 kW (`chChargeWatt`) under-stated every plan.
...
- **A ledger row with no stored window** is unscoreable, and **v1.132.0 distinguishes the two reasons**: a *pre-v1.39.0* row predates the window columns (a data-vintage problem), while a current-`algo_version` row with no window simply had **no cheap window to resolve that night** — normal on a tariff day that offers none, e.g. Saturday on this plan. Both capture as `scored=0`, now with different `score_notes`; calling the second a "pre-v1.39.0 row" sent readers hunting a migration bug that was not there. Nights whose raw telemetry has aged past the configured samples retention (default 30 days) capture honestly unscored rather than staying open forever.
...
| Message-rate floor | Per-SN hour-of-day EWMA baselines; collapse < 0.2× baseline sustained 20 min → edge-triggered P3 alert. **v1.95.0**: the dwell is chosen from the device's OWN profile — a FLAT device (cv ≤ `MSG_RATE_FLOOR_FLATNESS_MAX_CV` = 0.15 across ≥ 18 mature hour buckets, e.g. the SHP2 at cv ≈ 0.022) fires on `MSG_RATE_FLOOR_FLAT_COLLAPSE_MIN` = 4 min instead. The 20-min dwell exists only to separate legitimate Core idle (4.4 msg/min) from a collapse (2.1-2.9), a 1.5× problem created by the Cores' 13× diurnal swing; a flat device has no such ambiguity, and making it wait 20 min let the SHP2 run at 0.20 msg/min for ~17 min with nothing firing (5 instances in 4 days). Flatness must be EARNED — thin evidence keeps the conservative dwell. **v1.95.0** also gates on presence: a device that is actually OFFLINE no longer surfaces a collapse (the offline/stale alert owns that; the detector's subject is "barely reporting while still appearing FRESH"). Eligibility from a 7-day-half-life high-water mark (`eligibilityPeak`) so a collapse cannot disarm the detector; a fired collapse clears only on dwelled traffic ≥ max(0.2× baseline, `minBaselineRate`) — losing eligibility is never reported as recovery (§13.5) | Cumulative MQTT message counters | Alert monitor self-alert (defeats the "looks fresh" wedge trap) | measured-and-active |
...
v1.157.0 keeps this exception exactly: when idle Cores are excluded from the quorum
(`selfHealQuorum`), the panel still counts by identity, so a panel-only wedge heals on
the same schedule.
