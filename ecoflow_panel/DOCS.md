...
| `HOST_POWER_ENTITY` | _(empty)_ | Self-monitor for the Pi that runs this alarm — the whole monitor's single point of failure. Point it at HA's Raspberry Pi Power Supply Checker `binary_sensor` (device_class `problem`, on = kernel under-voltage), e.g. `binary_sensor.rpi_power_status`. When set, the add-on raises a `warning` alarm (`host-power-undervoltage`) if the host reports under-voltage, so a marginal supply or sagging circuit is caught before it browns the Pi out and takes the alarm dark. Empty (default) = dormant. (v1.6.0) |
...
binary_sensor:
  - platform: ping
    host: 192.168.5.21        # Core 1's LAN IP (set a DHCP reservation)
    name: Core 1 LAN
    count: 3
    scan_interval: 30
  - platform: ping
    host: 192.168.5.22        # Core 2's LAN IP
    name: Core 2 LAN
    count: 3
    scan_interval: 30
```
