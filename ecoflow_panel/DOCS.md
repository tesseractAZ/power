...
**v1.182.0 — dashboard figures cover the home.** `computeTotals` adds `fleet.homeCoverage` (home Cores' and the panel's series only; a dark home Core counts 0) and `fleet.panelLoadCoverage`; the Today card reads them ("not measured today" at 0). `computeDegradation`, `computeChargeCurveFingerprint` and `computeThermalEvents` tag packs `home` (`tagHomePacks`, panel membership); the Battery page scopes fleet capacity, the SoH range and the charge-curve/thermal lists to home packs (`web/src/cards/batteryScope.ts`). The Alerts badge (`alertBadge`) counts a learned CRITICAL. `energyFlowModel.membershipUnknown` keeps the Batteries title honest while the panel's source list is unknown. The night-charge status carries `reserveWriteMaxPct`; the card shows the clamped write (`reserveWriteLabel`) and clears an actuation banner by `actuationBannerVisible` (6 h after the revert). Harness: `scripts/mutate-dashboard-audit-four.mjs` (8 mutants).
