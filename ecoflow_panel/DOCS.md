...
**v1.183.0 — the last three audit findings.** `web/src/cards/cardText.ts`: `outlookOf` (no projection → "—", neutral), `strategyExclusion` (`loadIsEnable: false` is the load-strategy flag from `LoadStrategyCfg`, not the relay — "not in the SHP2's load strategy", with the power drawn; no strike-through), `remainSuffix` (the DPU countdown is "to full" / "to empty" by `batAmp` sign). `web/src/charts/axisFormat.ts`: `kwTick` for the Forecast chart's labelled kW axis and `compactTick` for the trend charts, whose unit moved from every tick to the axis label. Harness: `scripts/mutate-audit-last-three.mjs` (6 mutants).
