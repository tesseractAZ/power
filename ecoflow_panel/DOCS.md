...
| `BROADCAST_ANNOUNCE_VOLUME` | `off` | Announce volume for the Music Assistant `play_announcement` call. `off`/`none`/`standing` **omit** `announce_volume` entirely so MA plays at the speaker's standing volume — recommended for ecobee thermostats, which drop MA's set/restore volume dance (set the ecobee speaker loud device-side once). A number `0`–`100` pins an explicit announce volume; empty falls back to `BROADCAST_VOLUME × 100`. (v0.15.4) |
