...
**Deferred audit follow-ups — Alerts provenance, energy-reporting correctness, Solar PV coverage.** Picks up the tracked follow-ups from the v0.42.0/v0.43.0 audits. Two of the prescribed fixes landed as specified; one ("lifetime charge==discharge: split by sign") was investigated and found to rest on an incorrect root-cause hypothesis — the honest finding is documented below rather than a fabricated change.
...
- Tests: +11 across the three slices (`alertReserveSource.test.ts` reserve-band provenance/dedup ×4; `energyTotals.test.ts` RTE clamp + zero-input + boundary-counted-once ×4; `aggregator.test.ts` pvCoverage PV-only/empty/degenerate ×3). Full server suite 677 → 688; web `tsc -b + vite build` clean. RTE clamp and lifetime equality were live-recomputed against the running add-on.
