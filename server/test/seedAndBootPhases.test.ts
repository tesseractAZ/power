...
const { createRecorder, seededDeviceDarkMs } = await import('../src/recorder.js');
...
test('★ the boot-phases line comes AFTER the seed and accounts for the whole call', () => {
  ...
  assert.ok(phaseIdx > seedIdx,
    `the FIRST boot-phases line (#${phaseIdx}) must follow the seed (#${seedIdx}); v1.153.0's "99.99%" came from one that did not`);
  const m = lines[phaseIdx].match(
  ...
  assert.ok(m, lines[phaseIdx]);
  const parts = m!.slice(1, 8).map(Number);
  const total = Number(m![8]);
  const sum = parts.reduce((a, b) => a + b, 0);
  assert.ok(Math.abs(sum - total) <= parts.length, `the phases (${sum} ms) must account for the total (${total} ms)`);
  assert.ok(total <= callMs + 1, `the recorder cannot report more time (${total} ms) than the call took (${callMs.toFixed(1)} ms)`);
});
