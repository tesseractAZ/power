...
test('owner floor: the [10,50] envelope is the device clamp, proven in v1.164.0', () => {
  assert.equal(clampReserveTarget(20), 20, 'the owner floor is inside the envelope');
  assert.equal(clampReserveTarget(9), 10);
  assert.equal(clampReserveTarget(60), 50, 'the SHP2 settled at 50 when asked for 90');
