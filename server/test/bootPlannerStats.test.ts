...
/**
 * v1.163.0 — planner statistics at boot, DRIVEN against a real node:sqlite database.
 *
...
test('★ an index with NO statistics is analyzed at boot — in full, not under a sampling bound', () => {
  ...
  // "5000 401 …" — the truncated stats every boot from v1.153.0 through v1.162.0 wrote.
...
const SHAPES: Shape[] = [
  ...
  // v1.163.0 rebase — added to main by v1.156.0's realized-GHI capture (weatherRealizedGetStmt,
  // recorder.ts). Same full-equality shape as 'weather sample exists', a different projection:
  // (sn, metric, ts) is the composite index's entire key, so the lookup is a direct seek and
  // there is nothing for statistics to choose between. Found by this file's own coverage guard,
  // which failed the rebase with "17 statements … this test lists 16".
  { name: 'realized GHI capture probe', sql: 'SELECT rowid AS id, value FROM samples WHERE sn = ? AND metric = ? AND ts = ? LIMIT 1', params: ['WEATHER', 'ghi_wm2_realized', T0],
    sites: [[R, '`SELECT rowid AS id, value FROM samples WHERE sn = ? AND metric = ? AND ts = ? LIMIT 1`']] },
